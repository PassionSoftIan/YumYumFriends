...
class OpenViduVideoComponent extends Component {
  ...
  checkFacePosition(predictions) {
    ...
    predictions.forEach((prediction) => {
      ...
      const noseTip = prediction.landmarks[5]; // 코끝 좌표
  ...
  drawMask = (predictions) => {
    ...
    predictions.forEach((prediction) => {
      ...
      // const noseTip = landmarks[1];
      const noseTip = prediction.landmarks[5]; 
      ...
      context.drawImage(
        ...
        offsetY - maskHeight * 0.8 - 80,
        maskWidth,
        maskHeight
      );
      // context.save();
      // context.translate(offsetX, offsetY);
      // context.rotate(Math.atan2(rightEye[1] - leftEye[1], rightEye[0] - leftEye[0]));

      // context.drawImage(this.mask, -width / 2, -height * 0.8 - 40, width, height);
      // context.restore();
    });
  };
  ...
  detectFace = async () => {
    ...
    const predictions = await this.model.estimateFaces(frame, false);
    frame.dispose();
