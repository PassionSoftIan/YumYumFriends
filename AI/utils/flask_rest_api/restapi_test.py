...
def predict(model):
    ...
    if request.files.get('image'):
        ...
        if model in models:
            results = models[model](im, size=416)  # reduce size=320 for faster inference
            df = results.pandas().xyxy[0]
            # print(df)
            # print(type(df))
            # print(df.loc[0,'confidence'])
            check = False
            for num in range(len(df)):
                if df.loc[num, 'confidence'] > 0.7:
                    check = True
            if check:
                return {'eat' : 1}
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--port', default=1557, type=int, help='port number')
