...
const ProfilePage: React.FC = () => {
  ...
  const acquiredYums: Yum[] = yums.filter((yum) => acquiredYumIds.includes(yum.id));
          ...
          <div className="modal-content">
            ...
            <div className="yum-list">
              {acquiredYums.map((yum: Yum) => (
                <div
                  key={yum.id}
                  className={`yum-item${selectedProfilePicture === yum.name ? " selected" : ""}`}
                  onClick={() => handleProfilePictureSelect(yum)}
                >
                  <img src={require(`../assets/StopYums/${yum.name}.png`)} alt={yum.name} />
                </div>
              ))}
            </div>
